...
import { Plus, Trash2, Edit3, Save, X, Wrench, BookOpen, Rss, BarChart3, Share2 } from 'lucide-react';
import { store, ServiceItem, CourseItem, BlogPost, StatItem, SocialLink } from '../store';
...
function CRUDManager<T extends { id: string }>({
  ...
  useEffect(() => { setItems(getData()); }, []);
  const save = (data: T[]) => { setItems(data); setData(data); };

  const add = () => {
    const newItem: any = { id: Date.now().toString() };
    fields.forEach(f => { newItem[f.key] = f.type === 'number' ? 0 : f.type === 'boolean' ? false : f.key === 'details' ? [] : ''; });
    setForm(newItem);
    setEditing('new');
  };
  ...
  return (
    <div>
                ...
                <div key={f.key as string} className={f.rows ? 'md:col-span-2' : ''}>
                  ...
                  {f.type === 'boolean' ? (
                    <label className="flex items-center gap-2 text-white/50 text-sm">
                      <input type="checkbox" checked={(form as any)[f.key] || false} onChange={e => setForm({...form, [f.key]: e.target.checked} as any)} className="w-4 h-4" />
                      {f.label}
                    </label>
                  ) : f.rows ? (
      ...
      <div className="space-y-3">
        ...
        {items.length === 0 && (
          <div className="text-center py-16 text-white/20"><Icon size={40} className="mx-auto mb-3" /><p>Hozircha ma'lumot yo'q</p></div>
        )}
      </div>
    </div>
  );
...
export function ServicesManager() {
  return (
    <CRUDManager<ServiceItem>
      title="Xizmatlar" icon={Wrench} color="#8B5CF6"
      getData={store.getServices} setData={store.setServices}
      fields={[
        { key: 'title', label: 'Nomi' },
        { key: 'icon', label: 'Icon nomi (Globe, Smartphone, etc)' },
        { key: 'description', label: 'Tavsif', rows: 2 },
      ]}
      renderItem={(item) => (
        <div>
          <h3 className="text-white font-bold">{item.title}</h3>
          <p className="text-white/40 text-sm">{item.description}</p>
          <span className="text-[#C4A1FF]/40 text-xs">Icon: {item.icon}</span>
        </div>
      )}
    />
  );
...
export function CoursesManager() {
  return (
    <CRUDManager<CourseItem>
      title="Kurslar" icon={BookOpen} color="#7C3AED"
      getData={store.getCourses} setData={store.setCourses}
      fields={[
        { key: 'title', label: 'Nomi' },
        { key: 'lessons', label: 'Darslar soni', type: 'number' },
        { key: 'hasCertificate', label: 'Sertifikat', type: 'boolean' },
        { key: 'description', label: 'Tavsif', rows: 2 },
      ]}
      renderItem={(item) => (
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h3 className="text-white font-bold">{item.title}</h3>
            <span className="px-2 py-0.5 rounded-full bg-[#9B6DFF]/10 text-[#C4A1FF] text-xs">{item.lessons} dars</span>
            {item.hasCertificate && <span className="px-2 py-0.5 rounded-full bg-[#00D68F]/10 text-[#00D68F] text-xs">Sertifikat</span>}
          </div>
          <p className="text-white/40 text-sm">{item.description}</p>
        </div>
      )}
...
export function BlogManager() {
  return (
    <CRUDManager<BlogPost>
      title="Blog" icon={Rss} color="#A78BFA"
      getData={store.getBlog} setData={store.setBlog}
      fields={[
        { key: 'title', label: 'Sarlavha' },
        { key: 'date', label: 'Sana' },
        { key: 'content', label: 'Mazmun', rows: 4 },
      ]}
      renderItem={(item) => (
        <div>
          <div className="flex items-center gap-3 mb-1">
          ...
          <p className="text-white/40 text-sm line-clamp-2">{item.content}</p>
        </div>
      )}
    />
  );
...
export function StatsManager() {
  ...
  useEffect(() => { setItems(store.getStats()); }, []);
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-black bg-gradient-to-r from-[#00D1FF] to-[#C4A1FF] bg-clip-text text-transparent flex items-center gap-3">
          <BarChart3 size={32} className="text-[#00D1FF]" /> Statistika
        </h1>
        ...
        {editing && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="glass-card rounded-2xl p-6 mb-6 overflow-hidden">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div><label className="text-white/40 text-xs mb-1 block">Qiymat</label><input className="input-field text-sm" value={form.value || ''} onChange={e => setForm({...form, value: e.target.value})} placeholder="3+" /></div>
              <div><label className="text-white/40 text-xs mb-1 block">Label</label><input className="input-field text-sm" value={form.label || ''} onChange={e => setForm({...form, label: e.target.value})} placeholder="Yil Tajriba" /></div>
              <div><label className="text-white/40 text-xs mb-1 block">Icon</label><input className="input-field text-sm" value={form.icon || ''} onChange={e => setForm({...form, icon: e.target.value})} placeholder="Briefcase" /></div>
            </div>
            <div className="mb-4"><label className="text-white/40 text-xs mb-1 block">Tafsilotlar (har bir qator alohida)</label><textarea className="input-field text-sm" rows={4} value={form.detailsText || ''} onChange={e => setForm({...form, detailsText: e.target.value})} /></div>
            <div className="flex gap-3">
              <button onClick={saveEdit} className="btn-primary"><Save size={16} /> Saqlash</button>
              <button onClick={() => { setEditing(null); setForm({}); }} className="btn-ghost"><X size={16} /> Bekor</button>
            </div>
        ...
        {items.map((item, i) => (
          <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="stat-card p-6 group relative">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div><p className="text-white/70 font-medium">{item.label}</p><p className="text-white/30 text-xs">Icon: {item.icon}</p></div>
            </div>
...
export function SocialsManager() {
  return (
    <CRUDManager<SocialLink>
      title="Tarmoqlar" icon={Share2} color="#C4A1FF"
      getData={store.getSocials} setData={store.setSocials}
      fields={[
        { key: 'platform', label: 'Platforma' },
        { key: 'url', label: 'URL' },
        { key: 'icon', label: 'Icon nomi (Instagram, Send, etc)' },
      ]}
      renderItem={(item) => (
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-[#9B6DFF]/10 border border-[#9B6DFF]/20 flex items-center justify-center">
            <Share2 size={18} className="text-[#C4A1FF]" />
          </div>
          <div>
            <h3 className="text-white font-bold">{item.platform}</h3>
            <p className="text-white/30 text-xs">{item.url}</p>
          </div>
        </div>
      )}
    />
